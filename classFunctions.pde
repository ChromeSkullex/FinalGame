...
//How prepared the user is with the material. Out of 100. Everyone starts with 70. Decrease if class is missed, always increase with class.
int testClass1 = returnRanInt(50,70);
int testClass2 = returnRanInt(50,70);
int testClass3 = returnRanInt(50,70);
...
String ignoreClass(int i){
  String varClass = classHour[i]+":00 --- "+classNames[i] +" | "+classTest[i]+" | "+ classEff[i]+"\n";
  return varClass;
}

String stringClassTable(){
  String Title ="Time | Class | Prepare | Efficiency\n\n";
...
//returning the final grade for each class using efficiency

void returnFinalGrade(){
  //chance of subtraction or addition
  //refer to IgnoreThisCalculations
//println("result -> " + addSub());
  detAmt();
...
void checkMissedClass(){
  ...
  if(checkClass){
    ...
    textY+=TEXT_HEIGHT;
    //everytime you miss class, your efficency decrease
    decreaseTestPre(classSession, returnRanInt(10,20));
    decreaseEff(classSession, returnRanDec(10,20));
    text("You feel like you missed something important...",textX,textY);
    textY+=TEXT_HEIGHT;
